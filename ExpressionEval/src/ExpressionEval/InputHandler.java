...
public class InputHandler {
	/**
	 * Helper class for parsing left parenthesis. If the left parenthesis
	 * belongs to a log operator, |parenthesis| should be set to '[', otherwise '('. 
	 * 
	 * For log parenthesis, |hasComma| indicates whether the log has met a matching comma in
	 * the expression yet. 
	 * 
	 * |idx| represents the index of the parenthesis in the
	...
	/**
	 * Given a filename, read the file line by line and put each line into a list of string.
	 * @param filename	Absolute path of the file on disk.
	 ...
	 * @param formula	Expression in string.
	...
	/**
	 * Helper to parse operator could be both unary and binary +, -.
	 * The following situations are not valid if the operator is unary:
	 * 		two consecutive +
	 * 		three consecutive +/-
	 * @param currChar 		The char needs to be parsed.
	 * @param currTokens		All the tokens parsed before currChar.
	 * @return Parsed token.
	 */
	...
	/**
	 * Helper to parse operator /, *, ^. If the operator does not have two operands, this function would throw IllegalArgumentException.
	 * @param currChar 		The char needs to be parsed.
	 * @param currTokens		All the tokens parsed before currChar.
	 * @return Parsed token.
	 */
	...
	/**
	 * Helper to parse (. If there are implicit multiply (i.e. 2(3+4), (3+4)(5+6)), this function would throw IllegalArgumentException.
	 * @param startIdx		 The index of currChar in the original expression.
	 * @param currChar 		 The char needs to be parsed.
	 * @param parathesisStack The stack keeps track of the current opening parenthesis.
	 * @param currTokens		 All the tokens parsed before currChar.
	 * @return Parsed token.
	 */
	private static ExpressionCommon.Token parseLeftParenthesis(int startIdx, char currChar, Deque<LeftParenthesis> parathesisStack,
	...
	/**
	 * Helper to parse ). If there are binary operator right before the right parenthesis, this function would throw IllegalArgumentException.
	 * @param startIdx		 The index of currChar in the original expression.
	 * @param parathesisStack The stack keeps track of the current opening parenthesis.
	 * @param currTokens		 All the tokens parsed before currChar.
	 * @return Parsed token.
	...
	/**
	 * Helper to parse ,. If there are no matching log operator or the log operator already has a comma, this function would throw IllegalArgumentException.
	 * @param parathesisStack The stack keeps track of the current opening parenthesis.
	 * @return Parsed token.
	 */
	 ...
	 * @param chars        Charr array of the original expression.
	 * @param startIndex   The start index of the number.
	 * @param endIndex     An array with size = 1 to store the (end index of the number + 1).
	 * @return Parsed number in double and the end index would be stored in the array |endIndex| passed in.
	 */
	 ...
	 * @param currChar Character to be evaluated.
